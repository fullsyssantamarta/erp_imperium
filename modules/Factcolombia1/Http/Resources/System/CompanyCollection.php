    ...
    public function toArray($request)
    {
        return $this->collection->transform(function($row, $key) {
            return [
                ...
                // 'plan' => $row->plan->name,
                'plan_started_at' => $row->plan_started_at ? $row->plan_started_at->format('Y-m-d') : null,
                'plan_expires_at' => $row->plan_expires_at ? $row->plan_expires_at->format('Y-m-d') : null,
                'plan_auto_renew' => (bool) $row->plan_auto_renew,
                'plan_active' => (bool) $row->plan_active,
