...
<fe:DebitNote
    ...
    <cbc:ProfileID schemeDataURI="http://www.dian.gov.co/face/profileList.html">Nota débito, todos los perfiles</cbc:ProfileID>
    <cbc:ID schemeName="identificador del DebitNote asignado por el vendedor">{{$document->prefix}}{{$document->number}}</cbc:ID>
    ...
    @php
        // Consolidate health fields for debit notes
    ...
    @foreach ($document->detail_documents()->get() as $key => $detail)
        <cac:DebitNoteLine>
            <cbc:ID schemeName="identificador para el DebitNoteLine">{{$index++}}</cbc:ID>
            <cbc:UUID schemeName="UUID para el DebitNoteLine"/>
            <cbc:Note/>
